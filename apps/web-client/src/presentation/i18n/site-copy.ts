...
const COPY: Record<Locale, ClientCopy> = {
  en: {
    ...
    hero: {
      ...
      title: 'FrameShift converts and compresses batches on the server.',
      subtitle:
        'Upload JPG, PNG, WebP, AVIF, TIFF, GIF, HEIF, JP2, JXL, PDF, or SVG files, resize them with presets, compress them on the API, and download the results locally. The backend strips metadata and handles the heavy lifting.',
      badges: ['Bulk upload', 'Server-side processing', 'Resize presets', 'Metadata removed'],
    ...
    queue: {
      ...
      emptyText: 'Add a batch to start converting. Processing happens on the NestJS backend.',
      progressReady: 'ready',
  ...
  vi: {
    ...
    hero: {
      ...
      title: 'FrameShift chuyển đổi và nén hàng loạt ngay trên server.',
      subtitle:
        'Tải lên file JPG, PNG, WebP, AVIF, TIFF, GIF, HEIF, JP2, JXL, PDF hoặc SVG, đổi kích thước bằng preset, nén qua API và tải kết quả về máy. Backend sẽ loại bỏ metadata và xử lý phần nặng.',
      badges: ['Tải lên hàng loạt', 'Xử lý phía server', 'Preset kích thước', 'Đã bỏ metadata'],
    ...
    queue: {
      ...
      emptyText: 'Thêm một batch để bắt đầu chuyển đổi. Việc xử lý diễn ra ở backend NestJS.',
